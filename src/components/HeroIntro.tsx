        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 text-center sm:text-left">
            <button
              ...
              className="w-20 h-20 sm:w-24 sm:h-24 rounded-xl overflow-hidden shadow-lg shadow-purple-500/20 flex-shrink-0 ring-2 ring-purple-500/30 hover:ring-purple-400 hover:scale-105 transition-all cursor-pointer"
            >
              <img
                src="/profile.png"
                alt="Aaron Dsilva"
      ...
      {isImageExpanded && (
        ...
        >
          <div className="relative max-w-lg w-full mx-4 animate-in zoom-in-95 duration-200">
            ...
            <button
              onClick={() => setIsImageExpanded(false)}
              className="absolute -top-3 -right-3 w-8 h-8 bg-slate-800 hover:bg-slate-700 rounded-full flex items-center justify-center text-slate-400 hover:text-white transition-colors border border-slate-700"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
            <p className="text-center text-slate-400 text-sm mt-4">Press ESC or click anywhere to close</p>
          </div>
        </div>
      )}
    </>
